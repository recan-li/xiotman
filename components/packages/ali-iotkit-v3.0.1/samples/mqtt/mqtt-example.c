...
int smart_room_remote_set_parse(const char *topic, const char *payload)
{
    if (strstr(topic, "thing/service/property/set")) {
        cJSON* root = cJSON_Parse(payload); 
        cJSON *params = cJSON_GetObjectItem(root, "params");
        cJSON* ACRunningState_item = cJSON_GetObjectItem(params, "ACRunningState");
        cJSON* LightSwitch_item = cJSON_GetObjectItem(params, "LightSwitch");
        if (ACRunningState_item) {
            rt_kprintf("ac operation ...\n");
            smart_room_ac_on_off(ACRunningState_item->valueint);  
        }
        if (LightSwitch_item) {
            rt_kprintf("light operation ...\n");
            smart_room_light_on_off(LightSwitch_item->valueint); 
        }
        cJSON_Delete(root);
    }
    return 0;
...
static void example_message_arrive(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
    ...
    switch (msg->event_type) {
        case IOTX_MQTT_EVENT_PUBLISH_RECEIVED:
            ...
            rt_kprintf("\n");
            smart_room_remote_set_parse(topic_info->ptopic, topic_info->payload);
            break;
        default:
            break;
    }
...
static int example_subscribe(void *handle)
{
    ...
    ///a1Yiwd28Pw2/recan-test-device/user/get
    ///sys/a1Yiwd28Pw2/recan-test-device/thing/service/property/set

    rt_kprintf("sub topic: %s\n", topic);
...
static int example_subscribe2(void *handle)
{
    int res = 0;
...
static int example_subscribe3(void *handle)
{
    int res = 0;
...
///sys/a1Yiwd28Pw2/${deviceName}/thing/event/property/post
int mqtt_upload_msg(const char *payload)
{
    int             res = 0;
...
< }
*/
int aliyun_mqtt_upload_smart_room_info(smart_room_info_t *info)
{
    ...
    cJSON_AddStringToObject(root, "id", "1554386089"); 
    cJSON_AddStringToObject(root, "version", "1.0.0"); 
    cJSON *params = cJSON_CreateObject();
    if (NULL == params) {
        goto err;
    }
    cJSON_AddNumberToObject(params, "ACRunningState", info->ac_status);
    cJSON_AddNumberToObject(params, "LightSwitch", info->light_status);
    cJSON_AddNumberToObject(params, "RoomTemp", (double)info->t / 10.0);
    cJSON_AddNumberToObject(params, "RoomHumidity", (double)info->rh / 10.0);
    cJSON_AddItemToObject(root, "params", params);

    char * cjson_str = cJSON_Print(root);  //将json转化为字符串,
    rt_kprintf("cjson_str: %s\n", cjson_str);
    mqtt_upload_msg(cjson_str);
    HAL_Free(cjson_str);
...
static int mqtt_example_main(int argc, char *argv[])
{
    ...
    res = example_subscribe2(pclient);
    if (res < 0) {
        IOT_MQTT_Destroy(&pclient);
        return -1;
    }

    res = example_subscribe3(pclient);
    if (res < 0) {
        IOT_MQTT_Destroy(&pclient);
        return -1;
    }
    
    g_ali_mqtt_client = pclient;

    while (1) {
        if (0 == loop_cnt % 20) {
            //example_publish(pclient);
            //aliyun_mqtt_upload_smart_room_info(smart_room_get_info());
        }
...
static void mqtt_example_main_entry(void)
{
    mqtt_example_main(1, "sample");   
}

int aliyun_mqtt_init(void)
{
    rt_thread_t thread;

    thread = rt_thread_create("mqtt", mqtt_example_main_entry, NULL, 3072, 10, 20);
    if (thread != RT_NULL)
    {
    ...
    {
        EXAMPLE_TRACE("create wifi thread failed");
        return (-RT_ERROR);
    }
